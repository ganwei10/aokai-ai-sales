...
- **300 家客户 × 20 字段**：安省 180（60%）/ 美中 120（40%），中小型企业（Tier2/3）切入。
- **100 家渠道商**：安省 60 / 美中 40，含类型、专长、服务半径、七三开佣金（30%）。
- 每条客户带**招聘缺工信号**（红/黄/绿），可与第四阶段自动化联动。
- 支持按区域 / 层级 / 品类 / 信号 / 关键词实时筛选，状态可推进，并**一键导出 CSV / Excel（300×20）**。
- 数据由确定性生成器产出（`src/lib/db/generate.ts`），刷新即恢复，生产可替换为真实数据或 CRM 接入。
